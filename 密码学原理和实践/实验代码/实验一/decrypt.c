...
int derive_key_from_password(const char* password, unsigned char* derived_key, size_t key_len) {
    ...
    OSSL_PARAM params[] = {
        OSSL_PARAM_construct_octet_string("pass", (char*)password, strlen(password)),
        OSSL_PARAM_construct_octet_string("salt", salt, strlen((char*)salt)),
...
int decrypt_file(const char* input_filename, const char* output_filename, unsigned char* key) {
    ...
    unsigned char in_buffer[BUFFER_SIZE + 16]; // +16 是为了处理可能的填充
    int bytes_read;
    ...
    if (EVP_DecryptFinal_ex(ctx, final_buffer, &final_len) != 1) {
        EVP_CIPHER_CTX_free(ctx);
        fclose(input_file);
        fclose(output_file);
...
int main(int argc, char* argv[]) {
    ...
    if (decrypt_file(input_filename, output_filename, derived_key) != 0) {
